...
export async function seedDatabase(prisma: PrismaClient) {
  ...
  const prevReport = await prisma.report.create({
    data: {
      ...
      kpiValues: {
        create: honinKpis.filter((k) => prevVals[k.name]).map((k) => ({ kpiItemId: k.id, kpiName: k.name, unit: k.unit, ...prevVals[k.name] })),
      },
    },
  });
  ...
  const currReport = await prisma.report.create({
    data: {
      ...
      originalText: `【溝の口本院 週次報告 2026/6/16-6/22】
報告者：佐藤院長
...
■良かったこと
・問診ロープレを毎日やった結果、初診の成約率が上がった。
・楽トレの更新も声かけを増やせた。

■課題
・離反率が悪化。LINEは送ったが予約確保まで手が回らなかった。
・受付の人員が薄い時間帯がある。

■来週やること
・離反予測者リストを作って担当者別に予約確保する。
・楽トレ更新確認を週1で固定化する。
...
よろしくお願いします。`,
      ...
      kpiValues: {
        create: honinKpis.filter((k) => currVals[k.name]).map((k) => ({ kpiItemId: k.id, kpiName: k.name, unit: k.unit, ...currVals[k.name] })),
      },
      inflows: {
        create: [
  ...
  await prisma.feedback.create({ data: { reportId: currReport.id, sendStatus: "UNSENT" } });
  ...
  const counts = {
    ...
    kpiItems: await prisma.kpiItem.count(),
    kdiItems: await prisma.kdiItem.count(),
